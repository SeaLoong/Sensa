...
### 问题 G：WebUI 会把 `ok=false` 的动作结果误报为“操作成功”

继续复查 WebUI 时，重点检查了当前**没有 Intiface 环境**的机器上，测试页里 `连接 Intiface` 这类动作的前后端配合。结果确认：后端此前虽然会返回 HTTP 200，但 JSON 里可能是 `{ ok: false }`；而前端 `postAction(...)` 之前只按 HTTP 状态码判断是否成功，没有检查返回体里的 `ok` 字段。

**影响：**

- 在当前这类没有 `intiface-engine.exe` 的环境里，点击“连接 Intiface”时，界面可能错误弹出“操作成功”；
- 同类问题也会影响 `TCode connect` 之类返回 `{ ok: false }` 的动作；
- 用户会误以为设备连接已经成功，排障方向会被带偏。

**处理：** 已让前端对动作接口返回体里的 `ok` 做显式判断；若为 `false`，则走失败 toast 而不是成功 toast。同时补充了连接接口的返回消息，并优先使用前端本地化文案，避免中文界面直接显示英文后端错误串。
...
8. 修复 WebUI 对动作结果的判定逻辑，避免 `ok=false` 时仍误报“操作成功”。
...
### 无 Intiface 环境下的 WebUI 动作验证

已在当前**没有 Intiface 运行环境**的机器上启动 Companion，并直接验证：

- `POST /api/control/intiface/connect` 返回 `ok = false`，且 `connected = false`。
- WebUI 测试页点击“连接 Intiface”后，toast 正确显示为“操作失败”，并给出本地化说明，而不再误报成功。

这说明当前 WebUI 已能正确区分“HTTP 请求成功送达”与“动作本身执行成功”这两层语义。
...
3. **Intiface 实机验证仍受本地是否存在 `intiface-engine.exe` 与 Buttplug 设备影响。**
   - 本轮已验证“缺失时不阻塞系统”，且 WebUI 不会再把连接失败误报为成功；
