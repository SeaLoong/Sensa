...
public sealed class TCodeAxisConfig
{
    ...
    public int  MaxSpeed { get; set; } = 1400;
...
public sealed class SaveFile
{
    ...
    public void CopyFrom(SaveFile other)
    {
        ...
        var safety = other.Safety ?? new SafetyConfig();
        Safety.GlobalIntensityCap = safety.GlobalIntensityCap;
        Safety.RampUpMs           = safety.RampUpMs;
        Safety.Idle               = safety.Idle;
        Safety.EmergencyStopKey   = string.IsNullOrWhiteSpace(safety.EmergencyStopKey) ? "Escape" : safety.EmergencyStopKey;

        var rhythm = other.Rhythm ?? new RhythmConfig();
        Rhythm.Enabled  = rhythm.Enabled;
        Rhythm.WindowMs = rhythm.WindowMs;
        Rhythm.MinBpm   = rhythm.MinBpm;
        Rhythm.MaxBpm   = rhythm.MaxBpm;
