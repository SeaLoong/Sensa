...
namespace Sensa.Recording;

// ═══════════════════════════════════════════════════════════════════════
//  MotionRecorder — buffered capture of timestamped L0 samples
// ═══════════════════════════════════════════════════════════════════════

/// <summary>
/// Records (timestamp, L0) pairs from the motion runtime. Only stores frames while
...
/// </summary>
public sealed class MotionRecorder
{
    ...
    /// <summary>Called by the runtime whenever a new motion frame is emitted.</summary>
    public void Push(MotionFrame frame)
    {
        if (!_active) return;
        long relMs = Environment.TickCount64 - _startMs;
        lock (_frames) _frames.Add((relMs, frame.L0));
    }
...
// ═══════════════════════════════════════════════════════════════════════
//  FunscriptExporter — converts a MotionRecorder capture into a .funscript file
// ═══════════════════════════════════════════════════════════════════════
...
public static class FunscriptExporter
{
    ...
    /// </summary>
    public static string Export(MotionRecorder recorder)
    {
        var frames = recorder.TakeSnapshot();
