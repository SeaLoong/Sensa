...
public sealed record ScriptInputAction(long AtMs, float Pos01);
...
public sealed class ScriptInputPlayer
{
    ...
    public DeviceCommand Sample(double deltaMs)
    {
        lock (_gate)
        {
            AdvanceLocked(Environment.TickCount64);
    ...
    private ScriptInputSnapshot BuildSnapshotLocked()
    {
        ...
        return new ScriptInputSnapshot(
            Loaded: loaded,
            Playing: _playing,
    ...
    private static ParsedScript ParseFunscript(Stream stream)
    {
        ...
        var actions = new List<ScriptInputAction>();
        ...
        foreach (var actionElement in actionsElement.EnumerateArray())
        {
            ...
            actions.Add(new ScriptInputAction(atMs, pos01));
        }
