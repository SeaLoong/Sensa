...
| TCode 驱动 | 通过串口驱动 OSR2 / SR6 / OSR6，支持 3 轴 / 6 轴 |
| SafetySystem | 强度上限、启动渐变、紧急停止、空闲回中 / 回零 |
| WebUI | 浏览器内完成配置、状态查看、日志查看、录制与手动测试 |
...
5. 在 WebUI 中：
   ...
   - 配置信号路径与角色；
   - 连接 TCode 或 Intiface；
   - 使用“手动功能测试”在没有 VRChat 输入时直接验证设备动作；
   - 确认状态面板、日志面板与录制面板都正常工作。
...
- 服务状态面板（Loop / Emergency / BPM / Recording / Manual Test）
- TCode / Intiface 控制按钮
- WebUI / OSC / TCode / Intiface / Safety / Rhythm 配置编辑
- 信号配置表（新增 / 删除 / 修改 Role、Curve、Dead Zone 等）
- 手动功能测试（覆盖实时输出）
- 录制开始 / 停止 / 导出 `.funscript`
- 参数流表格（当前收到的 OSC 参数）
- 日志面板
- 轻量 3D-ish 设备姿态预览
...
**Q：TCode 连接失败怎么办？**  
A：确认 `ComPort` 正确、串口未被其他程序占用，并且设备驱动已正确安装。
